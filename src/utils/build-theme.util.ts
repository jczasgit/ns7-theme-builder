...
async function _build(theme: Theme) {
  ...
  const writeStream: WriteStream = fs.createWriteStream(path.join(theme.destination, filename), {
    flags: 'a',
    encoding: 'utf-8',
  });
  try {
    ...
    try {
      ...
      for (let i = 1; i < names.length; i += 1) {
        ...
        for (let j = 0; j < keyframes[names[i]].length; j += 3) {
          await _write(
            ws,
            `${keyframes[names[i]][j]} { ${keyframes[names[i]][j + 1]}: ${keyframes[names[i]][j + 2]} } \n`,
          );
        }
        await _write(ws, '} \n'); // * Closing bracket.
      }
